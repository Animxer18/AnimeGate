...
const Navbar = () => {
  ...
  const { data, error } = useSWR(`https://api.consumet.org/meta/anilist/advanced-search?query=${query}&type=ANIME&perPage=5`)
  ...
  return (
    <>
      <div className=''>
        ...
        <Link to="/" class="flex ml-4 items-center">
        <img src={AnimeGateLogo} class="h-6 mr-3 sm:h-9" alt="Flowbite Logo" />
        <span class="self-center text-xl font-semibold whitespace-nowrap dark:text-white">AnimeGate</span>
    </Link>
      ...
      <div class="quicksearchcontainer w-[500px] absolute right-32 top-[62px] rounded-md z-[6]   " >
        <div class="resultcontainer overflow-auto">
      {data !== undefined ? 
     ( data.results.map((item) => 
        <Link to={`/info/${item.id}`} class="p-2 pb-4 bg-[#25262d] hover:bg-[#505155] border-b-[1px]  border-white  block text-left">
        <div class="film-poster float-left ">
            <img className='w-[50px]'  src={item.image}/>
        </div>
        <div class="srp-detail pl-[4rem]">
            <h3 class="film-name font-bold" >{item.title.romaji}</h3>
            
                <div class="alias-name text-[13px] text-[#aaa]">{item.title.english} </div>
            
            <div class="film-infor text-[13px] text-[#aaa]">
